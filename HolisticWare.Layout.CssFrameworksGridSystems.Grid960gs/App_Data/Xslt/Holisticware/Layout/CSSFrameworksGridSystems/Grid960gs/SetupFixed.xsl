...
*	ThemeStyleHtmlHeadCustomization = ~/css/theme-style.css
	Default value = ~/css/theme-style.css
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:param name="pagespeedoptimized" select="translate($PageSpeedOptimized,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')" />
 	...
 	<xsl:param name="ThemeStyleHtmlHeadCustomization" select="/in:inputs/in:param[@name='ThemeStyleHtmlHeadCustomization']" />
	<xsl:param name="themestylehtmlheadcustomization" select="translate($ThemeStyleHtmlHeadCustomization,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')" />
  ...
  <xsl:template match="/">	
	<html>
	  <head>
		...
		<link
			rel="stylesheet" type="text/css" media="screen"
			href="{$themestylehtmlheadcustomization}" 
			/>			  
	  ...
	  <!--
		Do not put container div in setup XSLT - it cannoot be used
		Put it in page template
